...
/**
 * Phone PoC: 워치에서 오는 HR(Data Layer /hr) 수신 + 위치/고도/경사 표시.
 * adr-008 PoC. 날씨 제외.
 *
 * UI 원칙(디버깅 최적화):
 *  - 텍스트를 이어붙이지 않고 고정 카드(연결/HR/위치)를 실시간 in-place 갱신.
 *  - 1초 티커로 "n초 전" 신선도 갱신 + 5초 무수신 시 끊김(빨강) 표시.
 *  - 하단 이벤트 로그는 최근 N줄만 유지하고 항상 최신이 아래로 오도록 자동 스크롤.
 */
class PhoneActivity : AppCompatActivity() {
    ...
    private val onMessage = MessageClient.OnMessageReceivedListener { event ->
        if (event.path == "/hr") {
            val bpm = String(event.data).toIntOrNull() ?: return@OnMessageReceivedListener
            lastHr = bpm
            lastHrAt = SystemClock.elapsedRealtime()
            hrCount++
            ui.post { renderHr(); log("HR ← 워치  $bpm bpm") }
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        renderHr(); renderWatch(); renderFreshness()
        log("대기 중 — 워치에서 측정을 시작하세요")
    ...
    private fun buildUi(): ScrollView {
        ...
        col.addView(TextView(this).apply {
            text = "Zone2 Sensor PoC"
            textSize = 22f
            setTypeface(typeface, Typeface.BOLD)
            setTextColor(C_TEXT)
        })
        col.addView(TextView(this).apply {
            text = "워치 HR 수신 · 위치/고도/경사 · adr-008"
            textSize = 12f
    ...
    private fun bigValue(t: String, color: Int) = TextView(this).apply {
        text = t; textSize = 40f; setTypeface(Typeface.DEFAULT_BOLD); setTextColor(color)
    ...
    private fun renderHr() {
        if (lastHr < 0) {
            ...
            hrValue.text = "$lastHr bpm"; hrValue.setTextColor(C_HR)
        }
    }

    private fun renderWatch() {
        // refreshWatchNodes에서 실제 값 채움. 초기 placeholder만.
    }
    ...
    /** 1초마다: 마지막 수신 이후 경과/신선도 갱신. */
    private fun renderFreshness() {
        val now = SystemClock.elapsedRealtime()
        // HR
        if (lastHr >= 0) {
            val ago = ((now - lastHrAt) / 1000).toInt()
            val stale = ago > 5
            hrSub.text = if (stale) "⚠ ${ago}s 무수신 (끊김?)" else "방금 수신 · 총 ${hrCount}회 · ${ago}s 전"
            hrSub.setTextColor(if (stale) C_RED else C_GREEN)
        } else {
            hrSub.text = "수신 대기 (워치에서 측정 시작)"
            hrSub.setTextColor(C_MUTED)
        }
        // 위치
        if (lastFixAt > 0L) {
    ...
    private fun refreshWatchNodes() {
        Wearable.getNodeClient(this).connectedNodes
            .addOnSuccessListener { nodes ->
                if (nodes.isEmpty()) {
                    watchValue.text = dot(C_RED, "연결 안 됨")
                    watchSub.text = "워치 페어링/워치 앱 실행 확인"
                } else {
            ...
            .addOnFailureListener {
                watchValue.text = dot(C_RED, "노드 조회 실패")
                watchSub.text = it.message ?: ""
            }
    }
    ...
    private fun log(s: String) {
        ...
        logScroll.post { logScroll.fullScroll(ScrollView.FOCUS_DOWN) } // 최신이 아래
    }
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
        ...
        if (grantResults.firstOrNull() == PackageManager.PERMISSION_GRANTED) startLocation()
            ...
            locValue.text = "위치 권한 거부됨"
            locValue.setTextColor(C_RED)
            locSub.text = "위치/고도/경사 미표시"
        }
    }
