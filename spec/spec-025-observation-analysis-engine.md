...
- **상태**: Implemented (2026-07-10 — 10단계 완전 구현, 단위+통합 테스트 106건 통과[당시 누계, 현재 153건], APK 빌드 성공. AC-1~9 충족)
- **날짜**: 2026-07-10
...
| `DRIFT_MIN_N` / `DRIFT_FLOOR_MIN_N` | 30 / 20 | 드리프트 회귀 최소표본(SE 안정) / 개인 노이즈플로어 유의 최소관측 | 필드 |
| `CADENCE_WIN` / `CADENCE_MIN_N` | 60s / 10 | 케이던스 σ 창 / 최소표본 | 필드 |
| `PACE_RUN_LO` / `PACE_RUN_HI` | 2.5 / 12.0 min/km | 러닝 유효 페이스대(밖=걷기/정지로 분석 제외) | 필드 |
| `HRR_RECENT` | 30s | 노력 강도 기준(직전 평균 속도) 창 | 필드 |
...
### 9-1. 種類 C 튜닝 상수 대장 — 코칭 트리거 / 리포트 판정 / 경사 (2026-07-13 문서화)

CLAUDE.md "★ 근거는 반드시 문서에 남긴다" 준수 — §9 코칭/리포트 기능의 트리거/판정 상수를 여기 모아 근거를 선언한다. 모두 문헌 상수가 아니라 **UX/표시 튜닝을 위한 설계 선택(種類 C)**으로, 앵커 의미만 정직히 선언하고 필드 데이터로 조정한다.

**코칭 트리거(RunEngine/Coach)**

| 상수 | 값 | 의미 | 근거(앵커) |
|---|---|---|---|
| `LATE_SEC` | 1500s(~25분) | 반응 드리프트 코칭을 후반 페이싱 문구로 전환 | 일반 유산소 세션 후반 진입 근사 |
| 워밍업 큐 창/상승 | 45~90s / +25bpm | 초반 급상승 감지 → "천천히" 1회 | 워밍업 급상승 육안 기준 |
| 회복 인지/스로틀 | ΔHR < −0.3 bpm/s / 90s | 빠른 하강=회복 중 인지 | 완만한 회복 하강 기준 |
| 정지 감지/지속 | pace ≥ 19.0 min/km / 8s | 사실상 멈춤 → 독려 | 정지 sentinel 20 바로 아래 |
| 오르막 예방 경향 | uphillTendency > 0.4 | 학습 경향 임계(구간당 1회 예방) | 오르막 절반 가까이 초과면 예방 |
| 마일스톤 | 300s 배수 | Zone2 연속 5/10/15분 격려 | 5분 단위 동기부여 |
| 더위 밴드 HOT | tempC ≥ 28℃ | 더위 코칭 맥락 편입(방향 아님) | 열스트레스가 심박 드리프트에 유의해지는 통상 기준 |

**리포트/표시 판정 밴드** — 도출값(種類 A) 위에 얹는 "차이가 의미 있다고 볼 최소 폭". 측정 오차/일상 변동보다 크게 잡아 과잉 판정을 막는다.

| 상수 | 값 | 의미 | 위치 |
|---|---|---|---|
| 표시 드리프트 클램프 | −10~+25% (기준창 180~240s, 최소 30표본) | 참고 지표 헛값 억제(실제 통상 0~15%) | FeatureExtractor / Models.cardiacDriftPct |
...
| 상수 | 값 | 근거 |
|---|---|---|
| 회귀 창 `windowM` | 150m | GPS 고도 노이즈(±10~30m)를 평지 ±2%까지 누르려면 ~200m 필요 → 긴 창으로 거시 경향만 |
| 표시 EMA `emaAlpha` | 0.12 | 표시 안정화(작을수록 매끈) |
| 최대 경사 클램프 | ±15% | 러닝 지형 상한, 헛값 차단 |
| GPS 게이트 | 수평정확도 25m / 수직정확도 8m 초과 fix 버림 | 부정확 위치 배제(강건성) |

근거: 소비자 GPS 고도는 러닝 경사에 근본적으로 부적합(수직오차가 수평의 2~3배)이라, 긴 창 + 넓은 데드밴드(±2% 범주화)로 오르막/평지/내리막 범주만 신뢰하고 정밀 %는 주장하지 않는다. 상세 = `sensor/SlopeEstimator.kt` 헤더 주석(기압계는 후속 과제).
