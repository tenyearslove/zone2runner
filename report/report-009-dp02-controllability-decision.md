...
- 작은 LLM도 프롬프트를 대체로 따른다. 문제는 가끔 어긋난다는 것이다. 일반 챗봇이라면 넘어갈 수준이지만, 반대 방향 코칭이 한 번이라도 나가면 안 되는 이 앱에서는 "대체로"로는 부족하다.
- 그런데 코칭은 달리는 중에 실시간 음성으로 나간다. 사람이 문장 하나하나를 검사해 줄 수 없다. **통제는 사람 손이 아니라 자동으로 작동하는 구조여야 한다.** 그 구조를 고르는 것이 이 설계 결정이다.

### 2. 문제점

- **프롬프트만으로는 어긋난 문장을 막지 못한다.** 판정은 "초과 — 늦춰라"인데 문장은 "속도를 올려요"가 나올 수 있다. 다만 정답 방향은 LLM 밖의 규칙이 이미 정해 두었다 — 잘못 만들어지는 것은 못 막아도, 나온 문장을 정답과 대조해 잘못 나가는 것은 막을 수 있어야 한다.
...
| **장점** | 제어가능성 ★★★★★ (문장 기각(교정), 폴백(보완), 위험 시 LLM 우회(안전전환)가 구조에 있고 자동으로 작동한다)<br/>강건성 ★★★★★ (LLM이 실패해도 짧은 문구로 코칭이 이어진다) | 기능적응성 ★★★★ (규칙에 없는 상황에도 문맥을 읽은 코칭을 만들 잠재력)<br/>기능정확성 ★★★ (풍부한 문맥 반영 — 잠재력) |
