...
20~60초로 넓다. 그리고 **기온을 직접 안 쓴다.** 또 하나 — **경사(c) 개인화는 GPS 한계로 사실상 준고정**이다: GPS 고도
노이즈 + 경사 추정의 창 지연 + 언덕 구간이 대개 심박 상승(전이)이라 학습 표본에서 빠지는 세 요인이 겹쳐, 경사 보정은
문헌 값 수준에 머문다(속도 a/b는 정확한 GPS 속도로 잘 맞춰진다). 우리 방어는 — (1) Zone 2 대역이라 근사가 가장 잘 듣고,
(2) τ/드리프트/**a/b**를 **그 사람 데이터로 실시간 맞춰서** 사람 간 차이를 데이터가 흡수한다. 남는 빈틈(기온, 경사 개인화)은
향후 입력(기압계 등)으로 메울 검증된 경로다.
...
| P5 폰 혼자 매초 | 작은 LSTM이면 예측 자체는 가벼움 | 매초 순전파 + **상시 온라인 미세조정(역전파)** + 학습 파이프라인/모델 변환/버전 관리 공정 — 산술뿐인 1안보다 무거움 |
...
**장점(QA 별점)**: 기능적응성 ★★★★ 잠재(실제 ★★★ — 과적합 위험, §5-1) — 사실상 2안의 유일한 우위. (수행효율성은 소형 순전파는 가능하나 **상시 온라인 학습(역전파) 부하**로 ★★★, §5-1)
...
|---|---|---|
| 구조 | §3 다이어그램 | §4 다이어그램 |
| 장점 | 설명용이성 ★★★★★<br/>강건성 ★★★★★ | 기능적응성 ★★★★ 잠재(실제 ★★★, §5-1)<br/>(그 외 뚜렷한 우위 없음 — 수행효율성도 상시 온라인 학습 부하로 ★★★) |
...
- 2안 QA6(수행효율성)가 ★★★인 이유(2026-07-09 하향): 매초 소형 LSTM 순전파에 더해, 개인 어댑터를 **상시 온라인
  미세조정(역전파)**해야 한다 — 산술 몇 줄로 끝나는 1안(★★★★★) 대비 지속 연산/배터리 부하가 크다. 온디바이스 실행은
  가능하나 "효율적"이라 보긴 어렵다. (온디바이스 신경망은 보통 추론만 하고 학습은 오프라인인데, 2안은 학습을 상시 돌려야 함.)
...
- **경사(c) 식별성 처리(미결)**: GPS 경사 노이즈로 c 온라인 적합이 취약(준고정)임을 확인. 처리 방향 — 경사항을 문헌 고정 prior로 명시 / 조건부 게이팅 / 기압계 상대고도 도입 — 은 실주행 데이터로 결정(2026-07-09 보류).
