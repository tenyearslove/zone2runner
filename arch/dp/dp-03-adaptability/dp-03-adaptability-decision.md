...
- Zone 2 코칭의 기준선인 **개인 Zone 2 경계는 사람마다 다르고, 같은 사람도 체력에 따라 변한다.** 나이로 계산하는 공식은 출발점일 뿐이다 — 같은 나이라도 실제 최대심박은 공식에서 ±10bpm 이상 벗어나는 사람이 흔하다.
...
- **개인차가 크다**: 나이 공식의 개인 오차가 ±10bpm 수준이다. 경계가 10bpm 어긋나면 Zone 2 코칭은 다른 운동을 시키는 것과 같다.
...
- **온디바이스/비용 0**: 서버 학습 인프라 없이 폰 혼자 적응해야 한다(제약 C01).
...
|---|---|---|
| P1 | 나이 공식의 개인 오차가 ±10bpm 수준 — 고정 경계는 코칭을 틀리게 한다 | **기능적응성**(정체성 QA) — 개인별 수렴 |
...
| P6 | 폰 혼자, 서버/비용 0 | **C01, 수행효율성** — 무거운 재학습 불가 |
| P7 | 사용자가 적응을 되돌리거나 제어할 수 있어야 | **제어가능성**(DP-02) — 이동 한도/정정 채널 |

### 1-3. 설계 문제 (한 문장)

> **"개인차가 ±10bpm에 이르는 Zone 2 경계를 — 참값 없이, 드물고 주관적인 사용자 라벨만으로, 폰 혼자 — 몇 세션 안에 그 사람에게 수렴시키고 이후 안정되게 유지하는 적응 구조를 무엇으로 할 것인가."**
...
![1안 — 온라인 베이지안 개인 적응](../../diagrams/02b-component-cnc-simple.png)

> 채택안(1안)은 이 시스템의 실제 아키텍처이므로 일반 도식을 참조한다(그림 오른쪽 아래의 저장 + 개인 적응 학습 블록이 이 DP의 적응 루프). 상세 컴포넌트 뷰 = `arch/diagrams/02-component-cnc.png`, 컴포넌트 서술 = `arch/component-catalog.md`(A5/E1/F1/D3).
...
- **④ 사전값의 정직성**: 나이/안정심박 공식은 인구 평균으로 문헌 근거가 있는 출발점이며, 개인 오차(±10bpm 수준)가 크다는 것 역시 같은 문헌이 말한다 — 그래서 "출발선 + 개인 적응"의 역할 분담이 성립한다.
...
- **조건 의존성(공정한 평가)**: 이 별점은 *이 시스템의 조건*(단일 사용자/온디바이스/참값 없음/드문 라벨)에 대한 평가다. **조건이 다르면 — 수만 명 사용자, 서버 인프라, 랩 검증 코호트 확보 — 2안이 우선될 수 있다.**
...
- **제약과 맞는다.** 갱신이 닫힌 공식 몇 줄이라 폰 혼자, 비용 0으로 돈다(C01). 세션 데이터가 기기를 떠나지 않는다.
- **DP-01/DP-02와 서로 떠받친다.** 적응 상태(평균/확신 폭)가 그대로 노출되어 설명이 되고(DP-01), 이동 한도와 라벨 채널이 통제가 된다(DP-02) — 세 정체성 QA가 한 설계에서 나온다.
...
- **"공식(나이 기반)만으로 충분하지 않나?"** — 공식의 개인 오차가 ±10bpm 수준이라는 것이 문헌의 결론이다. 공식은 첫 세션의 출발선으로만 쓰고, 이후는 그 사람의 실제 답이 경계를 결정한다.

### 6-4. 보완 계획
- **수렴 지표 실측**: 시뮬(참임계를 아는 가상 러너)로 수렴 오차/안착 세션 수/경계 진동폭을 측정하고 있다 — 실주행 데이터로 같은 지표를 재측정한다(spec-002 QA2 측정, spec-012).
