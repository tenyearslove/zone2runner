...
코칭 AI는 달리기 중 참고할 안내를 낸다(존 판정, 개인 경계 적응, 코칭 큐) — 지시가 아니라 도움 도구다. 이 앱의 지향은 판단마다 "왜"를 풍부하게 붙이는 것인데, XAI의 함정이 여기 있다: **실제 판단과 어긋난 설명(plausible-but-unfaithful)은 잘못된 확신(false confidence)을 만든다.** 참값이 없어 설명이 "옳다"는 보장은 불가능하므로, 설계 문제는 "온디바이스에서, 비전문가에게, **충실도(faithfulness) 있게**(실제 판단 근거와 일치하게) 모든 결정을 설명하기".
