...
- 이 앱은 사용자가 **믿고 그대로 행동해야 하는** 결정을 낸다: "지금 존을 벗어났어요 / 당신의 Zone 2 상한은 여기 / 지금은 속도를 낮춰요".
- 기존 러닝 앱들은 대체로 이런 코칭의 **근거를 보여주지 않는다** — 왜 그렇게 하라는지 알 수 없다. 이 앱은 판단마다 "왜"를 붙이는 걸 핵심 품질(설명용이성)로 잡는다.
- **그런데 함정이 있다**: *그럴듯하지만 틀린 설명*은 설명이 없는 것보다 **나쁘다.** 사용자가 잘못 믿고(false confidence) 엉뚱하게 행동하기 때문이다.
- 즉 진짜 요구는 "설명이 있냐"가 아니라 "설명이 **충실하냐**(진짜 이유대로냐)"다. → **XAI 용어로 fidelity/faithfulness(충실도).**
...
### 6-2. 채택 근거 (2안 대비)
- **충실도(설명용이성) 우위**: 판단이 투명해 근거가 추적 가능하므로 *그럴듯하나 실제와 어긋남*의 여지가 작다. 심박처럼 사용자에게 보이는 값에서 어긋난 설명은 신뢰 저하로 직결되며(P2), 2안은 사후 근사이므로 이 실패모드에 노출된다. → **Rudin 2019의 고위험 처방과 일치.**
