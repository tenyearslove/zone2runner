...
- 이 앱은 달리는 동안 옆에서 말을 건네는 **러닝 코칭 앱**이다: "지금 존을 벗어났어요 / 당신의 Zone 2 상한은 여기예요 / 속도를 조금 낮춰볼까요". 코칭은 따라야 하는 지시가 아니라 참고용 안내다. 위험 심박 같은 안전 항목만 별도 규칙(안전 가드)이 맡는다.
- 이 앱의 지향은 분명하다: **판정/경계/코칭마다 "왜"를 풍부하게 붙이는 것**(정체성 품질 = 설명용이성). 기존 앱 대비 우위는 주장하지 않는다 — 벤치마킹 전이다(§6-4 보완 계획).
- **함정**: 설명을 많이 붙일수록 *실제 판단과 어긋난 설명*이 섞일 여지도 커진다. 어긋난 설명은 잘못된 확신(false confidence)을 만들 수 있다.
- 우리는 설명이 "옳다"고는 자신할 수 없다(참값 없음). 설계로 보장할 수 있는 것은 **"설명이 시스템이 실제로 쓴 값/규칙과 일치한다"** 는 것이다. → **XAI 용어로 fidelity/faithfulness(충실도).**
- **이 DP = 그 "충실한 설명"을 어떤 내부 구조로 보장할 것인가** 라는 설계 결정이다.
...
- **그럴듯함 ≠ 충실함**: 사람은 매끄러운 설명이면 믿는다. 그래서 *plausible-but-wrong*(그럴듯하나 틀린) 설명이 가장 위험하다 — 특히 심박처럼 사용자 눈에 보이는 값에서 어긋나면 신뢰가 즉시 붕괴한다.
...
| P2 | 실제 판단과 어긋난 설명은 잘못된 확신(false confidence)을 만든다 | **설명용이성 심화 = 충실도(fidelity)** |
...
> **"코칭 AI의 판정/경계/코칭을, 참값도 대량 데이터도 없이, 폰 혼자, 사용자가 믿고 정정할 수 있게, 그리고 무엇보다 *그럴듯함이 아니라 충실함(진짜 이유대로)*으로 설명하는 내부 구조를 무엇으로 할 것인가."**
...
## 3. (1안) 규칙/통계 기반 개인화 아키텍처 — 설계에 의한 설명(intrinsic) — ★채택
...
**LLM의 자리(중요)**: LLM은 결정에 관여하지 않는다. 규칙이 방향/사실을 확정한 뒤, LLM은 그 **확정된 사실을 자연어로 옮기기만** 한다(verbalizer). "왜"를 LLM이 지어내지 않으므로(P7), LLM 환각이 설명을 오염시키지 못한다. 표현 왜곡은 출력 가드레일(DirectionGuard)+템플릿 폴백이 막는다.

**각 수치의 출처 구분**: 화면의 숫자는 관측에서 온 값 / 학습된 값 / 설계로 정한 값이 구분돼 있어, 누르면 "이 값이 어디서 왔나"에 답할 수 있다(data lineage). 카운터와의 실질적 차이도 여기다 — 우리는 그 원리를 따라갈 수 있고, 블랙박스는 따라가기 어렵다.
...
## 4. (2안) 사전학습 블랙박스 아키텍처 — 사후 설명(post-hoc) — 카운터
...
- **조건 의존성(공정한 평가)**: 이 별점은 *이 시스템의 조건*(온디바이스/참값 없음/저차원 의미있는 특징/정체성 QA=설명용이성)에 대한 평가다. 조건이 다르면(고차원 원신호, 참값 확보 가능, 정확도가 최우선) 2안이 앞설 수 있다. 즉 "A가 늘 낫다"가 아니라 "이 조건에서 A가 낫다"이다.
...
- **제어가능성**: 라벨원이 사용자 입력(말하기 테스트)이라 "그건 내 답과 다르다"로 직접 정정 가능(HITL). 2안은 결정 경로가 표면화되지 않아 무엇을 정정할지 짚기 어렵다.
