...
**(3) 그래서 생기는 결론**
설명을 "결정과 분리된 사후 모듈"로 만들면, 그 설명은 원래 결정의 **근사/추정**이 되어 충실도를 보장할 수 없다. 충실한 설명을 얻는 확실한 길은 **결정 과정 자체를 투명하게 설계해서, 설명이 그 과정의 재생(replay)이 되게** 하는 것이다.
...
| P2 그럴듯하나 틀림 | 설명=실제 계산 재생이라 **어긋날 수가 없음**(설명과 결정이 같은 로직) |
...
- **③ 충실도(fidelity) vs 그럴듯함(plausibility)**: XAI 문헌은 사후 설명의 핵심 위험을 "그럴듯하나 충실하지 않을 수 있음"으로 명시하고, 충실도를 검증할 정답이 대개 없다고 본다(본 문서 P3와 동일). 1안은 재생 구조로 이 위험을 구조적으로 제거한다.
...
**장점(QA 별점)**: 설명용이성 ★★★★★ (설명=실제 계산 재생 → 충실), 제어가능성 ★★★★★ (정정 HITL + 출력 가드레일)
...
|---|---|---|
| 설명 방식 | 설계에 의한 설명(intrinsic) — 재생 | 사후 설명(post-hoc) — 근사 |
| 구조 | §3 다이어그램 | §4 다이어그램 |
...
| Trade Off | 설명이 **결정의 재생**이라 구조적으로 충실 — plausible-but-unfaithful 실패모드가 없음<br/>모든 수치 provenance(종류 A/B/C), 사용자 정정(HITL) 가능<br/>사후 계산 불필요 → 폰에서 가벼움 | 복잡한 비선형까지 데이터가 배울 잠재력, 고차원 신호에 강함<br/>단 그 정확 잠재는 참값이 없고 대상 대역 데이터가 부족한 이 조건에선 확인이 어려움<br/>설명은 근사라 안전/정정을 별도로 보강해야 함 |
...
- **1안 수행효율성 ★★★★★**: 설명이 이미 계산한 값의 재생이라 추가 연산이 거의 없다. 2안 ★★은 특징기여도 다중 재평가/상시 LLM 이유생성 부하 때문이다.
...
- **수행효율성**: 재생은 추가 연산이 거의 없다(P6). 2안의 사후 설명(특징기여도/LLM)은 상시 부하가 있다.
