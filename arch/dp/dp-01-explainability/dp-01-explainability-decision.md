...
>
> **상태**: v4 (2026-08-03, 리뷰 반영 — 근거 명확화 + 채택안 개별화 도식/상세 도식 신설). v3(2026-07-31) = 쉬운 문장 재작성(전문 용어는 부록 B로, 사용자 검토 반영). 리서치 노트(`dp-01-explainability-research.md`) 근거.
...
| P6 | 서버 없이 폰 안에서, 매초/세션종료, 비용 0 | **C01 온디바이스/비용0**, **수행효율성** — 무거운 사후 설명(특징기여도 다중평가/상시 LLM) 부담 |
| P7 | 약한 LLM이 설명에 끼면 그럴듯한 거짓을 지어낼 수 있다 | **제어가능성/강건성** — LLM 역할을 묶어야 |

### 1-3. 설계 문제 (한 문장)

> **"코칭 AI의 판정/경계/코칭을 — 정답지도 대량 데이터도 없이, 서버 없이 폰 안에서, 사용자가 정정할 수 있게 — 실제 판단과 항상 일치하는 설명으로 보여주는 내부 구조를 무엇으로 할 것인가."**
...
![1안 — 규칙/통계 기반 개인화 아키텍처(설명용이성 관점)](images/dp-01-explainability-adopted-simple.png)

> 위 도식은 실제 아키텍처(일반 도식 `arch/diagrams/02b-component-cnc-simple.png`)를 이 DP의 축(설명용이성)이 드러나게 개별화한 것이다. 상세 컴포넌트 뷰 = `arch/diagrams/02-component-cnc.png`, 컴포넌트 서술 = `arch/component-catalog.md`. 소스 = `images/dp-01-explainability-adopted-simple.puml`.
...
**채택안 상세 설계 — 추상 블록을 하위 모듈(실제 클래스)까지 풀어 그린 설명용이성 경로**:

![DP1 채택안 상세 — 하위 모듈 전개](images/dp-01-explainability-detail.png)

> 위 추상 도식의 «모델 서빙»/«출력 가드레일»/«설명 서비스»/«저장» 블록을 실제 클래스 단위로 전개한 것. 소스 = `images/dp-01-explainability-detail.puml`. 아래 표는 이 경로의 동작 순서 요약이다.
