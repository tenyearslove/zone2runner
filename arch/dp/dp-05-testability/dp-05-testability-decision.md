...
| 3. 순수 로직 분리 | 판정/개인화/분석/가드 로직이 기기(안드로이드) 의존 없는 순수 모듈 — 개발 머신에서 단위 테스트가 돌고(152개, 2026-08-01 기준), 변경마다 자동 회귀 검증 |
...
- **조건 의존성(공정한 평가)**: 이 별점은 *이 시스템의 조건*(1인 개발/참값 없음/잦은 로직 변경/위험 시나리오 포함)에 대한 평가다. **조건이 다르면 — 테스트 러너 풀과 장비를 갖춘 조직, 로직이 안정된 유지보수 단계 — 실주행 중심 검증의 비중이 커질 수 있다.**
...
- **"추상화 계층은 오버엔지니어링 아닌가?"** — RunSource는 메서드 몇 개의 인터페이스 하나이고, 시뮬 소스들은 그 구현일 뿐이다. 이 한 겹이 사는 것: 자동 회귀 152개, 위험 시나리오 검증, 수 주짜리 수렴 확인의 분 단위 압축. 비용 대비 산 것이 크다.
