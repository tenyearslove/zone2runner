...
| 3. 순수 로직 분리 | 판정/개인화/분석/가드 로직이 기기(안드로이드) 의존 없는 순수 모듈 — 개발 머신에서 단위 테스트가 돌고(146개), 변경마다 자동 회귀 검증 |
...
- **"추상화 계층은 오버엔지니어링 아닌가?"** — RunSource는 메서드 몇 개의 인터페이스 하나이고, 시뮬 소스들은 그 구현일 뿐이다. 이 한 겹이 사는 것: 자동 회귀 146개, 위험 시나리오 검증, 수 주짜리 수렴 확인의 분 단위 압축. 비용 대비 산 것이 크다.
