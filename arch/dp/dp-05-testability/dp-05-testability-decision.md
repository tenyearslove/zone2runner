...
>
> **상태**: v2 (2026-08-03, 리뷰 반영 — 채택안 개별화 도식 + 상세 도식 신설). 리서치 노트(`dp-05-testability-research.md`) 근거.
...
![1안 — 소스 추상화 + 폐루프 시뮬 계측(테스트가능성 관점)](images/dp-05-testability-adopted-simple.png)

> 위 도식은 실제 아키텍처(일반 도식 `arch/diagrams/02b-component-cnc-simple.png`)를 이 DP의 축(테스트가능성)이 드러나게 — 입력 원천을 가리는 RunSource 포트를 중심에 두고, 그 위에 자극 주입, 아래에 관찰을 놓아 — 개별화한 것이다. 실센서 자리에 시뮬 소스가 그대로 끼워지고 나머지 경로는 동일하다. 소스 = `images/dp-05-testability-adopted-simple.puml`. 상세 컴포넌트 뷰 = `arch/diagrams/02-component-cnc.png`, 컴포넌트 서술 = `arch/component-catalog.md`(A1/A9/F2).

**채택안 상세 설계 — 검증 경로를 하위 모듈(실제 클래스)까지 풀어 그린 도식**:

![DP5 채택안 상세 — 검증 경로 하위 모듈 전개](images/dp-05-testability-detail.png)

> 위 추상 도식의 포트/어댑터/참값 러너/순수 로직/관찰 블록을 실제 클래스 단위로 전개한 것(RunSource 인터페이스와 구현 5종, VirtualRunner의 참임계와 아티팩트 주입 필드, JVM 테스트 153개, RunActivity 시뮬 계측 패널). 소스 = `images/dp-05-testability-detail.puml`.
...
| 구조 | §3 다이어그램(실제 아키텍처를 테스트가능성 관점으로 개별화 + 상세 전개) | §4 다이어그램 |
...
> 관련 문서: `dp-05-testability-research.md`(리서치), `arch/component-catalog.md`(1안=우리 시스템 A1/A9/F2)/`dp-05-testability-counter-catalog.md`(2안 카운터), `images/`(1안 개별화 채택안 도식 + 상세 도식, 2안 카운터 도식)/`arch/diagrams/`(일반 아키텍처 도식), `spec/spec-002`(QA5 시나리오/측정), `spec/spec-003`(RunSource), `spec/spec-019`(가상 러너 검증 도구), `spec/spec-022`(수동 가상 러너), `spec/spec-012`(실주행 검증 계획), `arch/adr-024`(시뮬 역할 한정), `ml/EXPERIMENT_LOG.md` §15-3(짜고치기 확인 이력), `arch/dp/dp-01-explainability/`~`dp-04-robustness/`(같은 구조의 다른 QA 단면).
