...
>
> **상태**: v1 (2026-07-23, 사용자 검토 대기). 리서치 노트(`dp-02-controllability-research.md`) 근거.
...
**한 줄 개념**: 코칭의 **방향/사실은 규칙이 확정**하고, LLM은 그 확정된 내용의 **표현만** 맡는다. LLM 출력은 규칙이 재검증하고(출력 가드레일), 실패하면 템플릿으로 갈아타며(폴백 체인), 안전 권고는 LLM을 거치지 않고, 모든 호출의 경로/기각/폴백이 기록된다(감사).
...
| 층 | 장치 | 제어 유형(강의 3유형) |
|---|---|---|
| 1. 구조적 격리 | 방향/사실은 규칙이 확정, LLM은 표현만(verbalizer). 1순위 경로는 과제형 API(Rewriting: 입력=규칙이 확정한 문장, 톤만 변형)라 자유도가 더 좁음 | 예방(권한 자체를 좁힘) |
| 2. 출력 가드레일 | 형식 가드(길이/문장 수/이모지) + DirectionGuard(문장이 규칙 방향과 일치하는지 검증, 불일치 시 기각) | 교정 |
| 3. 폴백 체인 | Nano 톤 재작성 → Prompt 자유 표현 → 규칙 템플릿. 모든 경로 가용성 체크 + 무손상 폴백 — LLM 전면 실패 시에도 코칭 무중단 | 안전전환(fail-safe) |
...
| P3 미가용/지연/오류 | 가용성 체크 + 타임아웃 + 무손상 폴백 체인(층3) — 최악의 경우 = 규칙 코칭(현행 동작) |
...
| P7 표현 굳음 | 통제를 층으로 나눠 표현 자유도를 단계적으로 허용(톤 재작성 → 자유 표현 → 템플릿) — 상한은 인정(§단점) |
...
**정직한 한계(먼저 인정)**: (1) **표현 자유도 상한** — 코칭 내용의 뼈대가 규칙 문구 집합에 묶이므로, 규칙에 없는 새로운 상황 표현은 규칙 추가 없이는 안 나온다(P7). (2) **과차단 가능성** — DirectionGuard가 정상 문장을 기각할 수 있다(보수적 검사). 기각률 기록으로 튜닝할 대상이다. (3) **통제 ≠ 내용의 옳음** — 규칙 자체가 틀리면 통제는 성립해도 내용이 틀린다. 내용의 교정은 HITL(말하기 테스트)과 DP-01의 정정 채널이 맡는다.

**장점(QA 별점)**: 제어가능성 ★★★★★ (교정/보완/안전전환 개입 지점이 구조에 존재 + 감사 가능), 강건성 ★★★★★ (미가용/오류에도 무손상 폴백으로 무중단)
**단점(QA 별점)**: 기능적응성 ★★★ (표현/내용의 자유도 상한 — 새 상황은 규칙 확장 필요), 수행효율성 ★★★★ (가드 검증/폴백 오버헤드는 경미하나 존재)
...
| 단점 | 기능적응성 ★★★ (표현/내용 자유도 상한)<br/>수행효율성 ★★★★ (가드/폴백 오버헤드 경미) | 제어가능성 ★★ (개입 수단이 프롬프트 수정에 한정)<br/>강건성 ★★ (실패 시 판단 대체 불가) |
| Trade Off | 어긋난 출력은 규칙이 기각하고 실패 시 템플릿으로 무중단<br/>개입 지점(교정/보완/안전전환)이 구조에 존재 + 사후 감사 가능<br/>대신 코칭 내용의 자유도에 상한 | 판단+표현이 한 곳이라 구조 단순, 문맥 풍부한 코칭 잠재<br/>대신 통제가 "LLM이 규정을 따른다"는 가정에 의존<br/>실패/오작동 시 갈아탈 판단 주체가 없음 |
...
- **출력 가드레일**: 모델 출력을 시스템 규칙이 사후 검증/차단하는 장치. 이 시스템 = 형식 가드 + DirectionGuard(방향 일치 검증).
- **폴백 체인(fail-safe)**: 상위 경로 실패 시 하위 경로로 무손상 전환 — 톤 재작성 → 자유 표현 → 규칙 템플릿.
