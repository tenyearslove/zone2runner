...
**Proposed** — 두 방식을 `ml/`에 모두 구현해 실증 비교한 뒤 확정한다. 기존 A(직접 분류) 구현은 보존하고, B(HR 회귀)를 신규 구현한다. 비교 결과는 `ml/COMPARISON.md`.

예상 방향: 두 방식은 배타적이지 않다. **B의 개인화 HR 모델 + 임계 도출을 개인화 코어(DP3)로, A의 분류기를 다신호 판정(DP4)으로** 결합하는 하이브리드가 유력하다. 최종 결정은 실증 후.
