...
코칭 AI는 사용자가 **믿고 행동해야 하는** 결정을 낸다(존 판정, 개인 경계 적응, 코칭 큐). 그런데 XAI의 함정: **그럴듯하지만 틀린 설명(plausible-but-unfaithful)은 설명이 없는 것보다 나쁘다** — 잘못된 확신(false confidence)을 만들기 때문. 그래서 설계 문제는 "온디바이스에서, 비전문가에게, **충실도(faithfulness) 있게** 모든 결정을 설명하기".

- **결정축 = 충실도(fidelity)**: 그럴듯함(plausibility) ≠ 충실함(faithfulness). 코치처럼 사용자가 행동해야 하는 시스템에선 충실도가 지배한다. (Rudin 2019; 헬스-XAI 리뷰)

## 두 가지 설계 (비교)

### 설계 A(채택): 설계에 의한 설명 (intrinsic / glass-box + provenance)
투명한 규칙 판정 + 해석가능한 베이지안 경계(prior/σ 노출) + 모든 수치의 출처 추적(種類 A 도출 / B 학습 / C 설계선택 = data lineage) + **LLM은 이미 정해진 사실을 말로만 옮김(verbalizer)** + 출력 가드레일/템플릿 폴백.
- 설명 = 실제 계산의 재생(replay)이지 근사가 아님 → **구조적으로 충실**(설명과 실제 로직 사이 간극 없음).
...
| "없는 숫자 금지"(種類 A/B/C 출처 선언) | **provenance / data lineage** = 값의 출처를 구조적으로 보장 |
| 코칭: 규칙이 방향/사실 확정 + LLM은 표현만 + 가드레일/폴백 | intrinsic 결정 + **LLM = verbalizer(사실 재생, 이유 생성 아님)** |
