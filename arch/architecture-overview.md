...
> **⚠ 재작성 필요 (2026-07-10)**: 본 문서는 예측(HrOdeModel/생리 ODE, DP1)을 살아있는 컴포넌트로 서술하나, **예측은 드롭됐다**(adr-024). AI 방법도 자체 NN 0개로 재정립(adr-025). **정합 재작성 대상** — 예측 컴포넌트를 관측 데이터 분석 엔진(FR3)으로 교체하고, 전체를 **강의 AI System 아키텍처**(운영/추론/설명/분석 서비스 + 개발 서브시스템 + 입출력 가드레일 + HITL/HOTL + 적응 루프, `framework/ai-system-and-quality.md §1-3`)에 매핑한다. 현행 정본 = spec-001(FR)/spec-002(QA). 재작성 전까지 본문 예측 서술은 구식.
