...
**외부 의존 5종** = Health Services(워치 센서) / GPS / AICore(Gemini Nano) / Open-Meteo(기온, 세션당 1회) / OSM 타일 서버(지도 표시) — 자체 서버 없음, 러닝/개인 데이터 비이탈.
...
> 표의 AD 번호는 이 문서의 아키텍처 결정 번호다. **인증 보고서의 DP1~DP5(설명용이성/제어가능성/기능적응성/강건성/테스트가능성, `arch/dp/`)와는 별개 번호 체계**이니 혼동 주의.

| AD | 결정 | 문서 |
|:---:|------|------|
| AD0 | Watch-Phone Hybrid (Watch=수집/표시, Phone=판정/분석/코칭) | `adr-001` |
| **AD1** | **관측 데이터 분석 엔진** = 라이브 신호에서 자기참조 파생지표 도출(예측 대체) | `adr-024`, `spec-025` |
| **AD2** | 개인 Zone 2 경계 = 온라인 Bayesian 적응 (말하기 테스트 실라벨 주도) | `adr-004`, `spec-004` |
| **AD3** | Zone 2 판정 = 규칙 (ZoneJudge — 지속 심박 vs 개인 경계 + 히스테리시스) | `adr-013` |
| **AD4** | 코칭 = 규칙/엔진이 방향·사실 확정 + LLM 직생성(간결 프롬프트) + 출력 가드 3종 + 단어 수준 폴백 | `adr-002`/`adr-028` |
| **AD5** | 안전 가드 = 위험 심박 결정론 규칙 권고 (LLM 우회) | `spec-008` |

- **AD1 분석 엔진**(spec-025): 지표별 작은 모듈(DriftSlope/GapMinetti/CadenceStability/SubmaxHr/Hrr)을 레지스트리에 등록해 조립(OCP). 판단선은 bpm 상수가 아니라 도출 통계량(slope 대 k·SE) + 개인 노이즈플로어(m+k·σ̂, 온라인 EWMA) — 마법상수 금지. 손목 PPG는 정속서만 정확해 노이즈/정속 게이팅.
- **AD2 개인화**는 NN이 아니라 경량 Bayesian(소수/온라인 라벨, float 산술). 주 관측 = 말하기 테스트(검증 가능 실라벨), 디커플링은 Conconi 편향으로 다운웨이트.
- **AD3 판정**은 규칙(결정론, 모순 불가). 표시 존은 순간심박+히스테리시스(5존), 코칭/통계 판정은 60초 지속심박(adr-023 이중 기준).
- **AD4 코칭**: 사실·방향은 규칙/분석 엔진이 확정하고, LLM(Gemini Nano)이 그 사실로 문장을 직접 생성한다(간결 구조화 프롬프트, adr-028). 출력 가드 3종 = 형식(길이/이모지) + DirectionGuard(방향 잠금) + NumberGuard(숫자 무결성 — 출력 숫자 ⊆ 입력 사실). 미가용/기각 시 단어 수준 폴백 큐로 무중단. 모든 호출은 프로비넌스로 기록(spec-027).
...
HR 1~2초 수집   ──Wearable──▶       │
                 Data Layer         ▼
                        ...
                        ┌───────────┼───────────────────────┐
                        ▼           ▼                        ▼
             판정: ZoneJudge   관측 분석 엔진(AD1)      특징/개인화 관측
...
| QA2 기능적응성 | factor prior(adr-012)→온라인 Bayesian 경계(adr-004)→말하기 테스트 적응 + 개인 노이즈플로어 EWMA | 메커니즘 실증(시뮬 이월 루프 수렴 12.9→1.73bpm, 참임계/시드 스윕 15케이스 전부 1~2세션 안착 — report-015 QA2). 실측 임계추출 편향은 한계로 명시 |
...
| QA5 테스트가능성 | RunSource 추상화 + VirtualRunner 폐루프 + 수동 가상러너(spec-022) + 단위+통합 테스트 153건(2026-08-01, 품질 측정 6종 포함) | Watch 없이 전 파이프라인 실행 + 폐루프 통합검증 |
...
3. **3단계 (분석 엔진 전환)** — 완료(2026-07-10): 예측 제거 + 관측 분석 엔진 완전 구현(spec-025, 당시 누적 테스트 146건 — 현재 153건, APK 빌드). 잔여 = 실주행 검증(자기참조/실라벨은 검증 가능, 실인간 정확도는 미측정).
...
- 설계 결정: `adr-001`(AD0), `adr-002`(LLM 코칭), `adr-004`(Bayesian 경계), `adr-007`(LLM 검증), `adr-008`(HR 수집), `adr-009`(백그라운드), `adr-010`(지도), `adr-012`(콜드스타트 prior), **`adr-013`(판정=규칙)**, **`adr-023`(워치 무로직 뷰어)**, **`adr-024`(예측 드롭→분석 엔진)**, **`adr-025`(AI≠NN, NN 0개)**, `adr-017`(HRV 보류), `adr-026`(Nano 과제형 API — Rewriting은 adr-028로 폐지), **`adr-027`(모델 다운로드)**, **`adr-028`(LLM 직생성+단어 폴백)**
