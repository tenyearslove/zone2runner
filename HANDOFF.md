...
최종 갱신: 2026-07-04 (★ adr-014 문헌 근거 역치 추정 NN — 본래 목적 복원. 실기기 피드백 다수 반영. 폰/워치 설치)

> **★ adr-014 (2026-07-04): 개인 젖산역치 추정 NN — "왜 NN"의 문헌 근거 확보.**
> 핵심 고민: 심박 예측/판정 태스크는 ablation에서 NN이 선형에 밀림(시뮬 선형+실데이터 모두 검증, NN 이득 3~6%).
> → 방향 전환: NN을 **개인 젖산역치(≈Zone2 상단) 추정**에 사용. 문헌 근거 확보(Etxegarai 2018 RNN R=0.89 /
> Zhu 2025 전이학습 / Rogers 2021 DFA-α1). 논문 RNN을 **1인 과제용 소형 MLP로 경량화**(adr-014 경량화 매핑표).
> 결과: 세션특징 8종 → uFrac 회귀, **공식(0.70) 대비 MAE 6.09→4.73bpm(-22%)** = 정직한 방어점(baseline=통용 공식).
> 앱: ThresholdEstimator(Kotlin MLP) + LearnedZone(EMA 누적) → 다음 세션 prior. spec-015, train_threshold.py.
> **보고서 인용 문단은 adr-014 맥락에 완성형으로 있음**(실존 논문, 그들 수치는 그들 것으로 명시, "근사"로 표현).
> 사용자 방침: 엄밀 증명 불필요, 논문 근거로 그럴듯하면 됨(거짓말만 아니면). ablation 결과는 보고서 비노출.
