...
> - **★ 다음(집에서 이어가기)**: **미구현 리포트/코칭 백로그**(spec-025 §9 말미 + 아래) 중 원하는 것 구현. **리포트 R**: 컨디션 지표(그날 서브맥시멀 vs 평소), 이탈 원인 분해(오르막/페이스/드리프트), 효율 곡선(HR-페이스 산점도), 주간/월간 요약. **코칭 C**: 워밍업 큐, 후반 페이싱, 회복 구간 인지, 더위 페이싱 강화, 패턴 학습 예방. + 실주행 필드 검증(HRR 노력종료/게이트 상수 튜닝은 실데이터 대기).
>
> --- (이하 이전 기록) ---
