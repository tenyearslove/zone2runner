...
최종 갱신: 2026-07-08 세션 F (adr-023 워치 뷰어화 + spec-022 수동 가상러너 — 두 기기 설치 완료)

> **★★★★★ 2026-07-08 세션 F — 워치 순수 뷰어화(adr-023) + 수동 가상러너(spec-022), 실기기 설치 완료 [여기서 이어가기]**
...
> - **adr-023 (Accepted, adr-022 대체)**: (1) **표시 존 = 순간심박 + 히스테리시스**(DisplayZoneJudge: 경계 2bpm 초과 즉시 / ±2bpm 안 3초 연속 시 전환) — 큰 숫자와 색 일치. 코칭/통계/개인화는 60초 지속심박 유지(adr-013 역할 분리). (2) **워치 = 무로직 뷰어**: 폰이 `/run/live`로 "순간심박,존인덱스(1~5),존내위치(0~1000)" 푸시, 워치는 그대로 그림. 워치 자체 판정(Zones)/로컬 폴백/`/zones` 동기화(ZoneSyncService/ZoneSync) **삭제**. 폰 두절(8초)시 존 미표시 "폰 동기화 중"(숫자는 회색, 사용자 확정). (3) **토크테스트 타이밍 판단 폰 이관**: 폰 updateTalkPrompt(지속심박≥하한 30초+/3분 간격, 10분 폴백) → `/run/talk` 전송, 워치는 설문 표시+응답만. (4) **5존 색 폰-워치 통일**(DisplayZone hex = 워치 HrZone): 폰 칩 "Z3 유산소"식 표기, ZoneBandView Z3노랑/Z4주황/Z5빨강 분할 + 솔리드 마커=순간(존 기준), 얇은 틱=평균.
> - **spec-022 수동 가상러너(완전수동 시뮬)**: 시뮬 입력 3모드 순환(자동/수동 페이스/**수동 러너**). 신체 스펙 다이얼로그(나이/안정/최대/기준페이스, 기본=프로필) + **케이던스 140~200 / 보폭 0.60~1.50m / 심박 보정 -20~+20 슬라이더**(재생 중 실시간). 심박 = 속도(spm x 보폭)→강도 역산→1차 지연(τ30s)+드리프트+포화(ManualVirtualRunnerSource). **토크테스트 조건 충족 시 폰에도 5지선다 팝업**(30초 타임아웃) — 수동 러너 모드 전용. **"시계연동" 체크박스**: 체크 시에만 워치 미러/존/설문 전송(해제=폰 단독).
> - 테스트: DisplayZoneJudgeTest 7건 추가(분할/히스테리시스/존내위치). 앱 전체 테스트+빌드 통과, wear 빌드 통과. **폰(SM-S948N)+워치(SM-L330) 무선 adb 설치 완료.**
> - **실기기 미검증(다음 러닝/시뮬에서 확인)**: AC(spec-010 AC-7/8, spec-022 AC-1~6) — 특히 야외에서 폰-워치 존 색 일치, 수동 러너 슬라이더 반응, 시계연동 체크 동작.
