...
최종 갱신: 2026-07-05 저녁 (토크 5단계+보통+워치 타이밍 / 음성,호흡 객관화 탐색→기각→페이스 talk test PoC / 예측 온라인 개인 보정 / 리팩토링)
...
> **★ 토크테스트 단측 관측 정정(사용자 지적)**: '편한데 미달'에서 편함이 경계를 잘못 끌어내리던 결함 → **단측**(편함=임계≥현재→올림만/벅참=임계≤현재→내림만/보통=점). 홈 문구도 정정. spec-016 FR2b. 회귀 테스트 2건. 테스트 앱 64건.
> **★ 설계 결정(DP) 종합 재작성 (report-004)**: 사용자 요청 — 기존 DP 재검토 + 신규 도출을 하나로. 두 축 = ①무엇으로 푸는가(도구선택 DP0~4) ②어떻게 믿는가(DP5 검증=폐루프 가상러너/QA5, DP6 설명=규칙사실+LLM표현/QA6, DP7 무결성 가드레일 횡단). 관통원칙 "규칙이 사실/방향/정답 확정, 학습·생성은 그 안에서". **검토 후 결정할 것**: DP5/6을 adr-020/021로 정식화 + report-003 덱/spec-001 추적표 반영 여부(승인은 사용자 몫). WHITEPAPER도 최신화(단측/기온/QA6/프로필관리).
