...
1. 폰: Zone2Runner → 프로필에 나이/안정심박 확인(Zone2 경계가 여기서 나옴).
2. 워치: Zone2Runner 실행 → "시작" → 권한 3개 허용(심박/위치/알림) → HR 숫자 뜨는지.
   - **지속 알림("Zone2Runner")이 떠 있는지 확인** — 이게 백그라운드 유지의 증거(adr-009).
...
| 전부 실패 | 폰 단독 수동 가상러너 시뮬(spec-022)로라도 완주 — 코칭/TTS/리포트/로그 파이프라인은 검증됨 (구 "가짜 라이브" Mock 모드는 7e6d172에서 제거) |
...
3. 규칙 판정(ZoneJudge)이 실데이터에서 안정적인지(채터링), 코칭 타이밍/방향 적합성
4. Gemini Nano 야외 실사용(지연 1~2.7초는 시뮬로 실측 완료, 방향 적합성은 관찰 필요)
5. 개인화 uEst가 실세션에서 어떻게 움직이는지(QA2 기능적응성)
